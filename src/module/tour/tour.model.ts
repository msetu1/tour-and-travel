...
const tourSchema = new Schema<ITour, TTourModel, ITourMethods>({
  ...
  coverImg: { 
    ...
    validate: {
      ...
      message: 'Cover image must be a valid URL ending with .jpg, .jpeg, .png, or .webp'
    }
  },
  image: {
    type: [String],
    validate: {
      validator: function (v: string[]) {
        return v.every(img => /^https?:\/\/.+\.(jpg|jpeg|png|webp)$/.test(img));
      },
  ...
  startLocation: { 
    type: String, 
    trim: true, 
    maxlength: [100, 'Start location must have less or equal to 100 characters'] 
  },
  location: { 
    type: [String], 
    validate: {
